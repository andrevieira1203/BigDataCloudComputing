...
endpoints = {
    ...
    "12": {"name": "Upload Image", "url": "http://127.0.0.1:8080/upload_image", "method": "POST"},
...
def get_user_choice():
    ...
    if choice in endpoints:
        ...
        if "{}" in url_template:
            if choice == "13":  # Image Download
                image_name = input("Enter the name of the image to download (e.g., xray1.jpg): ").strip()

                if not image_name:
                    print("❌ Image name is required!")
                else:
                    # Direct URL for image in the bucket
                    signed_url = f"https://storage.googleapis.com/bdcc-imagebucket/xray/{image_name}"

                    # Sending request to download the image
                    response = requests.get(signed_url, stream=True)

                    if response.status_code == 200:
                        # Path where the image will be saved (on Desktop)
                        desktop_path = os.path.join(os.path.expanduser("~/Desktop"), image_name)

                        # Make sure the directory exists
                        os.makedirs(os.path.dirname(desktop_path), exist_ok=True)

                        # Saving the image on Desktop
